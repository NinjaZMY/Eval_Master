...
try {
  ...
  //simple Eval ain't working

  simpleEval = false;
  try {
    // trying trustedtypes Eval
    // Dynamic code string
    ...
    if (!supportsTS)
      throw new TypeError(
        "This browser or webpage doesn't support TrustedScripts "
      );
    ...
    /*bug below in evalFn */
    /*for testing below: */
    // x= (s, policy = Policy) => ((s==''?null:s)+":"+policy)
    // x("")
    ...
    error(
      " simple eval = " + simpleEval + " ; failed due to:",
      er,
      "trusted eval == ",
      ExecTrustedEval
    );
    ...
    error(
      "This website supports TrustedTypes => ",
      supportsTS,
      " => trustedtype Evaluation failed",
      err,
      "trusted eval == ",
      ExecTrustedEval
    );
